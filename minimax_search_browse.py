...
client = OpenAI(api_key=os.environ.get("MINIMAX_API_KEY"), base_url="https://api.minimax.io/v1")
...
def get_response(query, max_retry=2):
    for retry_cnt in range(max_retry):
        try:
            ...
            print(f"Get response {retry_cnt} error: {e}", flush=True)
...
def get_search_results(query, max_retry=3):
    ...
    for retry_cnt in range(max_retry):
        try:
            ...
            print(f"Search {retry_cnt} error: {e}", flush=True)
            time.sleep(random.uniform(1, 16))

    if source_text == "":
        if "\"" in query:
            message = "Search result for query [{}] is empty. Return search result for cleaned query instead.".format(query.replace("\n", r"\n"))
            ...
            print("Search result for query [{}] is empty".format(query.replace("\n", r"\n")), flush=True)
            source_text = "Search result is empty. Please try again."
    return source_text
    ...
    if len(tokenized_source_text) > token_limit:
        ...
        with ThreadPoolExecutor(max_workers=num_split) as executor:
            for i in range(num_split):
                ...
                source_text_i = tokenizer.convert_tokens_to_string(tokenized_source_text[start_idx:end_idx])
...
def get_browse_results(url, browse_query, max_retry=3):
    ...
    for retry_cnt in range(max_retry):
        try:
            ...
            print(f"Read jina {retry_cnt} error: {e}, url: {url}", flush=True)
...
if __name__ == "__main__":
    ...
    print(get_browses_results(["https://zh.wikipedia.org/zh-hans/%E4%B8%AD%E5%8D%8E%E4%BA%BA%E6%B0%91%E5%85%B1%E5%92%8C%E5%9B%BD", "https://zh.wikipedia.org/wiki/%E7%BE%8E%E5%9B%BD"], browse_query="装备"))